...
// =============================================================================
// Eip1559Tx RLP Encoding
// =============================================================================

types::Bytes rlp_encode(const Eip1559Tx& tx) {
  ...
  const size_t payload = chain_id_len + nonce_len + max_priority_fee_len + max_fee_len +
                         ...
                         y_parity_len + r_len + s_len;
  const size_t rlp_len = RlpEncoder::list_length(payload);
  const size_t total = 1 + rlp_len;  // 0x02 prefix + RLP

  // Phase 2: Encode
  types::Bytes result(total);
  result[0] = 0x02;  // Type prefix
...
// =============================================================================
// Eip1559Tx RLP Decoding
// =============================================================================

TxDecodeResult<Eip1559Tx> rlp_decode_eip1559_tx(std::span<const uint8_t> data) {
  // Note: data should be the RLP payload AFTER the 0x02 type byte
...
// =============================================================================
// Eip1559Tx Transaction Hash
// =============================================================================

types::Hash tx_hash(const Eip1559Tx& tx) {
  if (tx.cached_hash) {
...
// =============================================================================
// Eip1559Tx Signing Hash (for sender recovery)
// =============================================================================

types::Hash signing_hash(const Eip1559Tx& tx) {
...
// =============================================================================
// Eip1559Tx Sender Recovery
// =============================================================================

std::optional<types::Address> recover_sender(const Eip1559Tx& tx, crypto::Secp256k1Context& ctx) {
  if (tx.cached_sender) {
...
// =============================================================================
// Eip1559Tx JSON Encoding
// =============================================================================

std::string json_encode(const Eip1559Tx& tx) {
  std::ostringstream ss;
  ss << '{';

  json_append_key(ss, "type");
  json_append_string(ss, "0x2");
  ss << ',';
...
// =============================================================================
// Eip1559Tx JSON Decoding
// =============================================================================

JsonDecodeResult<Eip1559Tx> json_decode_eip1559_tx(std::string_view json) {
